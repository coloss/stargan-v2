...
@torch.no_grad()
def debug_image(nets, args, inputs, step, outdir = None):
    outdir = outdir or args.sample_dir
    image_path_dict = {}
    ...
    image_path_dict[Path(filename).stem[7:]] = filename
    ...
    if args.latent_dim > 0:
        ...
        for psi in [0.5, 0.7, 1.0]:
            filename = ospj(outdir, '%06d_latent_psi_%.1f.jpg' % (step, psi))
            translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename)
            image_path_dict[Path(filename).stem[7:]] = filename

    # reference-guided image synthesis
    filename = ospj(outdir, '%06d_reference.jpg' % (step))
...
@torch.no_grad()
def debug_image_paired(nets, args, inputs, step, outdir = None):
    outdir = outdir or args.sample_dir
